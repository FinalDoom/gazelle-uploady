...
(function (window, $, {HTML2BBCode}) {
  ...
  function fetchComplete(nintendo) {
  ...
  const japaneseTextRE = /[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]/;
  ...
  function getGameInfo() {
    ...
    nintendo.title = $('a.wiki-title').text().trim();
    nintendo.description = html2bb($('section .wiki-item-display.js-toc-content').absoluteLinks());

    nintendo.tags = $.map($(`#wiki-${gameId}-genres a`), (elem) => $(elem).text().trim().toLowerCase());
    nintendo.alternate_titles = [$('.wiki-item-display .aliases').text()];
    nintendo.year = $(`#wiki-${gameId}-release_date`).getYear();
    // #endregion Fetch wiki info

    // #region Fetch images
    nintendo.cover = $('.wiki-boxart img').attr('src');
    $.ajax({
      url: imagesUrl,
      success: (data) => {
        ...
        $.ajax({
          ...
          success: (data) => {
            nintendo.screenshots = data.images.map(({original}) => original);
            GM_setValue('nintendo', nintendo);
            ...
            $.ajax({
              ...
              success: (data) => {
                ...
                if (!TOC.size()) {
                  ...
                  $(`#wiki-${gameId}-platforms`)
                    ...
                    .click(function (event) {
                      ...
                      nintendo.platform = $(this).text();
                      $(this).css({border: ''});
                      fetchComplete(nintendo);
                    });
                ...
                TOC.find('a').click(function (event) {
                  ...
                  if (alternateReleases && nintendo.alternate_titles) {
                    nintendo.alternate_titles = nintendo.alternate_titles.concat(alternateReleases);
                  }
                  ...
                  if (releaseTitle && nintendo.title !== releaseTitle) {
                    // Add title to alternate titles and use platform title instead
                    if (nintendo.alternate_titles) {
                      nintendo.alternate_titles.push(nintendo.title);
                    } else {
                      nintendo.alternate_titles = [nintendo.title];
                    }
                    nintendo.title = releaseTitle;
                  }
                  nintendo.rating = releaseBlock.find('[data-field="rating"]').text().trim().replace(/^$/, 'N/A');
                  nintendo.platform = releaseBlock.find('[data-field="platform"]').text().trim();
                  nintendo.year = releaseBlock.find('[data-field="releaseDate"]').getYear();
                  ...
                  nintendo.description = html2bb(releaseInfo) + nintendo.description;
  ...
  function validateSearchedValues() {
    ...
    if (isNewGroup()) {
      ...
      $('#aliases').val(Array.from(new Set(nintendo.alternate_titles)).join(', '));
      $('#title').val(nintendo.title);
      $('#tags').val(Array.from(new Set(nintendo.tags)).join(', '));
